...
def run_exe(ligand_filepath):
    PQ_pdb_name = ligand_filepath.split("/patterns/")[1].split(".")[0]
    pdb_id = ligand_filepath.split("/patterns/")[1].split("_")[1]
    residue_id = ligand_filepath.split("/patterns/")[0].split("/")[-1]

    ccp4_filepath = str(CCP4_DIR.resolve()) + '/' + pdb_id + '.ccp4'
...
def get_filepathes(rootdir: str, ligand_type: str):
    # e.g. cannot find 6zsg.ccp4
    # not 
    l = []
    no_ccp4_available_file = str(NO_CCP4_AVAILABLE_FILE.resolve())
    ...
    for dirpath, dirs, files in os.walk(rootdir + '/' + ligand_type):
        for f in files:
            if f.endswith(".pdb") and f.split("_")[1] not in no_ccp4:
                l.append(os.path.join(dirpath, f))
    return l

# function that runs analysis for a specific ligand type
# ligand 
def run_analysis(args: argparse.Namespace):
    ...
    for ligand_type in list_of_dirs:
        ...
        with open(str(Path(OUTPUT_DIR / cvs_filename).resolve()), mode='w') as f:
            rows = []
            w = csv.writer(f, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            # this gets pathes for pdb files
            # should get filepathes only for that ligand type
            filepathes = get_filepathes(args.rootdir, ligand_type)
...
# download all ccp4 files based on content of input dir folder
